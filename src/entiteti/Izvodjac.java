...
public class Izvodjac {
	...
	public static List<Izvodjac> dohvatiIzvodjacePoAlbumu(int id) throws SQLException {
		...
		String query = "SELECT IZ.id_izvodjac, IZ.ime, IZ.prezime,IZ.tip " + "FROM music_studio.izvodjac as IZ "
				+ "WHERE IZ.id_izvodjac IN (SELECT id_izvodjac " + "FROM izvodjacalbum " + "WHERE id_album = ?)";
	...
	public static List<Izvodjac> dohvatiIzvodjacePoPjesmi(int id) throws SQLException {
		var con = DatabaseConnector.getConnection();
		String query = "SELECT IZ.id_izvodjac, IZ.ime, IZ.prezime,IZ.tip " + "FROM music_studio.izvodjac as IZ "
				+ "WHERE IZ.id_izvodjac IN (SELECT id_izvodjac " + "FROM izvodjacpjesma " + "WHERE id_pjesma = ?)";
		PreparedStatement ps = con.prepareStatement(query, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
		ps.setInt(1, id);
		ResultSet rs = ps.executeQuery();
		List<Izvodjac> izvodjaci = new ArrayList<Izvodjac>();
	...
	public static List<Izvodjac> dohvatiIzvodjacePoSpotu(int id) throws SQLException {
		var con = DatabaseConnector.getConnection();
	...
	public static Izvodjac dodajIzvodjaca(Izvodjac izvodjac) throws SQLException {
		var con = DatabaseConnector.getConnection();
		String query = "INSERT INTO Izvodjac(ime, prezime, tip) " + "VALUES (?, ?, ?)";
		PreparedStatement ps = con.prepareStatement(query, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
